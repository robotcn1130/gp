 ...
 * 计算技术指标
 */
function calculateTechnicalIndicators($klineData, $currentPrice) {
    $indicators = [];
...
// 发送进度信息：资金流向获取完成，正在获取K线数据...
sendProgress(45, '资金流向获取完成，正在获取K线数据...');
...
try {
    ...
    $technicalIndicators = calculateTechnicalIndicators($klineData, $currentPrice);
} catch (Exception $e) {
...
if (!empty($parsedReviewData)) {
    ...
    foreach ($parsedReviewData as $index => $record) {
        ...
        $userPrompt .= ($index + 1) . ". {$type}，价格：{$price} 元，时间：{$time}，剩余数量：{$remaining}\n";
    }
    $userPrompt .= "\n";
}
...
logDebug('保存分析记录结果: ' . ($saveResult ? '成功' : '失败'), $user['id']);
