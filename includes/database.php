...
class Database {
    ...
    public static function saveStockAnalysis($userId, $symbol, $shares, $sellable_shares, $cost, $cash, $model, $marketData, $indexData, $newsData, $aiContent, $sectorData = null, $moneyFlowData = null, $technicalData = null, $reviewData = null, $fundDirectorContent = null, $minuteData = null, $analysisId = 0) {
        ...
        if ($analysisId > 0) {
            ...
            // 只更新fund_director_content字段，避免其他数据被覆盖
            if ($hasFundDirectorColumn) {
                $updateFields[] = "fund_director_content = " . ($fundDirectorContent ? "'$fundDirectorContent'" : "NULL");
            }
            
            // 如果有更新字段，执行更新操作
            if (!empty($updateFields)) {
                $sql = "UPDATE stock_analyses SET " . implode(', ', $updateFields) . " 
                        WHERE id = $analysisId AND user_id = $userId";
                
                return $conn->query($sql) ? $analysisId : false;
            } else {
                // 没有字段需要更新，直接返回分析ID
                return $analysisId;
            }
            ...
            if ($hasSellableSharesColumn) {
                $columns[] = 'sellable_shares';
                $values[] = $sellable_shares;
            }
            ...
            if ($hasFundDirectorColumn && $fundDirectorContent) {
                $columns[] = 'fund_director_content';
                $values[] = "'$fundDirectorContent'";
            }
